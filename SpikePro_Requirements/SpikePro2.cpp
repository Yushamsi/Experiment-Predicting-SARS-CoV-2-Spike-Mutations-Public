...
//    }
    ...
    int readResult;
    std::vector<std::vector<char>>* querySequences = new std::vector<std::vector<char>>();
    std::cout << "\nReading query..." << std::endl;
    querySequences->push_back(std::vector<char>(fastaSequence.begin(), fastaSequence.end()));

    int numQueries = 1;
    int queriesTotalLength = (*querySequences)[0].size();
