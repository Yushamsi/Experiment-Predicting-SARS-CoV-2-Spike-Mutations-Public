...
// int readFastaSequences2(const char* sequence, vector< vector<char> >* seqs);

int readFastaSequences2(const std::string& sequence, std::vector<std::vector<char>>& seqs);
...
//    }
    ...
    std::string fastaSequence = argv[1];

    int readResult;
    std::vector<std::vector<char>> querySequences;
    std::cout << "\nReading query..." << std::endl;
    readResult = readFastaSequences2(fastaSequence, querySequences);
    if (readResult) {
        std::cout << "Error: Unable to read the fasta sequence." << std::endl;
        return 1;
    }
    ...
    std::cout << "\nRead query, " << queriesTotalLength << " residues total." << std::endl;


    char* targetFilepath = NULL;
    std::vector<std::vector<char>>* targetSequences = new std::vector<std::vector<char>>();
    ...
    std::priority_queue<int> bestScores; // Contains numBestSeqs best scores
    int k = kArg;
...
/** Reads sequences from a string in FASTA format.
 * @param [in] fastaString String containing the header and sequence in FASTA format.
 * @param [out] seqs Sequences will be stored here, each sequence as vector of letters.
 * @return 0 if all ok, positive number otherwise.
 */
int readFastaSequences2(const std::string& fastaString, std::vector<std::vector<char>>& seqs) {
    seqs.clear();
    ...
    for (char c : fastaString) {
        ...
        if (inHeader) { // I do nothing if in header
            ...
            if (c == '>') {
                ...
                inSequence = false;
                if (!header.empty()) {
                    seqs.push_back(std::vector<char>());
                    seqs.back().insert(seqs.back().end(), header.begin(), header.end());
                    header.clear();
                }
            } else {
                if (c == '\r' || c == '\n')
